...
## Overview
A Python (discord.py 2.4) Discord bot that manages tiered "account generation" (free / free+ / premium), stock, timed drops, subscriptions, vouches, invite tracking, and owner admin tooling. Built as a Cog-based bot with a lightweight JSON-file persistence layer — no external database required.
...
- On startup the bot loads all cogs (`generate`, `stock`, `drops`, `vouches`, `subscriptions`, `admin`, `profile`, `blacklist`, `stats`, `utility`) and syncs ~45 slash commands globally. Check the workflow console for `✅ Logged in as ...` / `✅ Bot is ready!` to confirm a clean start.
- Data is stored as JSON files under `data/` (or `$DATA_DIR` if set) — `config.json`, `stock.json`, `users.json`, `vouches.json`, etc. Writes are atomic (temp file + rename) so a crash mid-write won't corrupt a file.
...
- `database.py` — all JSON persistence (config, users, stock, drop stock, vouches, invites, blacklist, generate log, stock alerts, banner image).
...
- `subscriptions.py`, `vouches.py`, `profile.py`, `blacklist.py`, `stats.py`, `admin.py`, `utility.py` — the remaining command groups.
...
- `/vouch` is now rate-limited to 1/hour/user to deter review spam.
